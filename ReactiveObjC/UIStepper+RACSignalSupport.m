// 该文件实现了 UIStepper (RACSignalSupport) 分类，提供了基于 ReactiveObjC 的响应式步进器值监听能力。
//
// 主要功能：
// 1. 提供 rac_newValueChannelWithNilValue 方法，便于监听 UIStepper 值变化，实现双向绑定。
//
// 实现思路：
// - 通过信号机制，将 UIControl 事件与 value 属性绑定，实现响应式数据流。
//
// 适用场景：
// - 需要响应式监听 UIStepper 值变化，或实现 MVVM 场景下的双向绑定。
//
