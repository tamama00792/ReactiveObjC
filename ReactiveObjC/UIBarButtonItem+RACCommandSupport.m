// 该文件实现了 UIBarButtonItem (RACCommandSupport) 分类，提供了基于 ReactiveObjC 的响应式按钮命令绑定能力。
//
// 主要功能：
...
// - 通过 hijack target-action 机制，点击时自动执行命令。
//
// 适用场景：
...
/**
 设置并绑定 RACCommand 到 UIBarButtonItem。
 ...
 4. 通过 hijack target-action 机制，点击时自动执行命令。
*/
- (void)setRac_command:(RACCommand *)command {
	...
	// hijack target-action，点击时自动执行命令
	[self rac_hijackActionAndTargetIfNeeded];
}

/**
 劫持点击事件，确保点击时自动执行 rac_command。
*/
