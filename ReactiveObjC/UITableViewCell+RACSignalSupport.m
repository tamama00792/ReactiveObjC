// 该文件实现了 UITableViewCell (RACSignalSupport) 分类，提供了基于 ReactiveObjC 的响应式复用事件监听能力。
//
// 主要功能：
// 1. 提供 rac_prepareForReuseSignal 方法，便于监听 cell 复用事件。
//
// 实现思路：
// - 通过信号机制，将 prepareForReuse 方法调用转为信号流。
//
// 适用场景：
// - 需要响应式监听 cell 复用事件，做清理或重置操作。
//
