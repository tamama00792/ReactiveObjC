// 该文件实现了 UITableViewHeaderFooterView (RACSignalSupport) 分类，提供了基于 ReactiveObjC 的响应式复用事件监听能力。
//
// 主要功能：
...
//
// 适用场景：
// - 需要响应式监听 Header/Footer 复用事件，做清理或重置操作。
//
...
/**
 返回一个信号对象，用于监听 Header/Footer 视图即将被复用的事件。
